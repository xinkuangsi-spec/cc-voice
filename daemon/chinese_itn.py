# coding: utf-8
# 取自 CapsWriter-Offline（MIT）core/server/engines/qwen_asr_gguf/inference/chinese_itn.py，
# 那边同样用它处理 Qwen3-ASR 的输出。相对原版的改动只有两处，都标了「cc-voice:」。
'''
...
def replace(original):
    ...
    try:
        ...
        # cc-voice: 副词「十分」（十分重要）不是 10 分钟，后面跟「钟」才转
        elif original == '十分' and string[r_pos:r_pos + 1] != '钟':
            num_type = '副词'
            final = original
        ...
        num_type = '错误'
        final = (head or '') + original      # cc-voice: 原版这里丢了 head 前缀
        if DEBUG:
